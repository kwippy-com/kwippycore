...
type = "private"
type_table = "kwippy_private_message"
fid = "5"
limit = 100
im_type = "2"
...
while (min_count<max_count):
     cursor.execute("""select kwippy_private_message.sender_id,kwippy_private_message.receiver_id,kwippy_private_message.message,auth_user.username,kwippy_user_profile.display_name from kwippy_user_profile,kwippy_private_message,auth_user where kwippy_user_profile.user_id=auth_user.id and auth_user.id=kwippy_private_message.sender_id and kwippy_private_message.id>=%s and kwippy_private_message.id<%s""",(min_count,min_count+limit,))
     commands = cursor.fetchall()
     for c in commands:
          cursor.execute("""select DISTINCT(kwippy_account.provider_login),kwippy_notificationsetting.im from kwippy_account,kwippy_notificationsetting where kwippy_notificationsetting.user_id=%s and kwippy_account.provider=%s and kwippy_account.user_id=%s LIMIT 1""",(c[1],im_type,c[1],))
          acc = cursor.fetchone()
          if acc:
              sett = acc[1].split(",")
              if sett[2]=='1':
                  # create the message string
                  mesg = c[4]+" has sent you a private message.<br>"+c[2]+"<br>p.s. you can reply privately here -<br>"+url_prefix+c[3]
