...
def do_sniptomore_profile(parser, token):
    bits = list(token.split_contents())
    nodelist = NodeList()
    return SnipToMoreProfileNode(bits[1])

def sniptomore_profile(parser, token):
    return do_sniptomore_profile(parser, token)

class SnipToMoreProfileNode(Node):
    def __init__(self, var1):
        self.var1 = Variable(var1)
    def __repr__(self):
        return "<SnipToMoreProfileNode>"
...
register.tag('sniptomore_profile',do_sniptomore_profile)
